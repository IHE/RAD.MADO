...
**Proposed resolution:** The arrows to the WADO endpoint will come from the Series boxes. We will add a new box representing a viewer with a error coming from the study. Allow WADO and XC-WADO endpoint on study as well as series level.
...
**Proposed Resolution**: Flag them using obligations.
...
** Proposed Resolution**: In order to provide a binding in a FHIR, a FHIR ValueSet is required. I propose the define it in FHIR and refer to the FHIR definition from the pdf.
...
**Proposed Answer:** This profile (and the study-instance-uid profile) are added to clearly define the type field requirements when providing identifiers that represent these concepts.
...
**Proposed Resolution:** remove the reference to performed procedure from the Bundle
...
**Answer:** In (an earlier?) version of the MADO spec, in the the KOS list of images, there was a remark that this should contain the representative images to be shown first. This extension was added to support those. I cannot find that text the current spec anymore. I these are indeed no longer in the spec, we will remove this extension.
...
**Answer:** These were included to show examples of the individual artifacts. We can remove them and only support the Bundles.
